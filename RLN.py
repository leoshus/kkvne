...
from kk.node_mdp import NodeEnv
...
class RLN:
    ...
    def train(self, training_set):
                    ...
                    if len(node_map) == req.number_of_nodes():
                        ...
                        if reward != -1:
                            ...
                            loss_value = self.sess.run(self.loss,
                                                       feed_dict={self.tf_obs: epx, self.input_y: epy})
                            ...
                            print("Failure!")
                    # 当实验次数到达batch size整数倍 累积的梯度更新一次参数
        ...
        with open('results/nodeloss-%s.ttx' % self.num_epoch, 'w')as f:
    ...
    def choose_max_action(self, observation, sub, current_node_cpu, acts):
        """在特定状态observation下，根据策略网络输出的概率分布选择概率最大的动作，仅利用"""
        x = np.reshape(observation, [1, observation.shape[0], observation.shape[1], 1])
